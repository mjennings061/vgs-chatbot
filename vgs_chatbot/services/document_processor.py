...
class RAGDocumentProcessor(DocumentProcessorInterface):
    ...
    async def _extract_text_content(self, document: Document) -> str:
        ...
        try:
            ...
            if document.file_type == "application/pdf" or file_path.suffix.lower() == '.pdf':
    ...
    def _enhance_pdf_text_structure(self, page_text: str) -> str:
        ...
        for line in lines:
            ...
            if (len(line) < 80 and
                (line.isupper() or
                 any(word in line.upper() for word in ['CHAPTER', 'SECTION', 'PART', 'APPENDIX']) or
                 line.endswith(':'))):
    ...
    def _split_text_into_chunks(self, text: str) -> list[str]:
        ...
        if weather_table_chunk:
            ...
            text_without_table = text.replace(weather_table_chunk, "\n[WEATHER_TABLE_REMOVED]\n")
        ...
        for i, line in enumerate(lines):
            ...
            if 'weather limitations table' in line_lower:
                ...
                for j in range(i + 1, min(i + 25, len(lines))):
                    ...
                    if ('crosswind' in next_line.lower() and 'kts' in next_line.lower()) or \
                       ('maximum wind speed' in next_line.lower()) or \
                       (('20kts' in next_line or '25kts' in next_line or '30kts' in next_line) and
                        len(next_line.split()) < 20):  # Short lines with wind data
                        end_idx = j
                    # Stop at major section break
                    elif (next_line.startswith('===') and 'weather' not in next_line.lower() and 'cfs' not in next_line.lower()) or \
                         next_line.startswith('DHO ') or \
                         'minimum cloud base' in next_line.lower():
                        break
    ...
    def _extract_metadata(self, content: str, document_name: str) -> dict[str, Any]:
        ...
        for line in lines:
            ...
            elif any(word in line.upper() for word in ['CHAPTER', 'SECTION', 'PART', 'APPENDIX']):
                sections.append(line)
            # Look for topic-specific sections
            elif any(word in line.upper() for word in ['WEATHER', 'OPERATIONS', 'LIMITS', 'PROCEDURES', 'EMERGENCY', 'SAFETY']):
        ...
        aviation_terms = [
            "wind limit", "wind speed", "crosswind", "headwind", "tailwind",
            "weather", "conditions", "limits", "maximum", "minimum", "operational",
            "knots", "kt", "mph", "m/s", "gusts", "turbulence", "visibility",
            "gliding", "glider", "aircraft", "operations", "safety", "emergency",
            "procedure", "altitude", "approach", "landing", "takeoff", "circuit"
        ]
        ...
        import re
        numeric_patterns = re.findall(r'\d+\s*(?:knots|kt|mph|m/s|feet|ft|meters|m)\b', content_lower)
        if numeric_patterns:
    ...
    async def search_documents(self, query: str, top_k: int = 5) -> list[ProcessedDocument]:
        ...
        try:
            ...
            print(f"ChromaDB search returned {len(results.get('ids', [[]])[0])} results")
        except Exception as e:
        ...
        for i, _chunk_id in enumerate(results["ids"][0]):
            ...
            directory_path = results["metadatas"][0][i]["directory_path"]
    ...
    async def index_documents(self, processed_docs: list[ProcessedDocument]) -> None:
        ...
        for doc in processed_docs:
            ...
            if not doc.embeddings or len(doc.embeddings) != len(doc.chunks):
                print(f"Warning: Document {doc.original_document.name} has invalid embeddings")
                continue

            for i, (chunk, embedding) in enumerate(zip(doc.chunks, doc.embeddings, strict=False)):
        ...
        print(f"Indexed {len(processed_docs)} documents with {sum(len(doc.chunks) for doc in processed_docs)} total chunks")
